...
export default class CodeGenerator {
  ...
  _handleKeyDown (selector, value, keyCode) {
    ...
    block.addLine({ type: domEvents.KEYDOWN, value: `await ${this._frame}.keyboard.press('Enter')\n` })
      ...
      block.addLine({type: domEvents.KEYDOWN, value:`await ${this._frame}.keyboard.press('Backspace')\n`})
  ...
  _handleApi (selector) {
    return new Block(this._frameId  , { type: pptrActions.RECORD_API, value: `try {await ${this._frame}.waitForResponse('${selector}',{timeout :1000}) } catch(err){}\n` })
